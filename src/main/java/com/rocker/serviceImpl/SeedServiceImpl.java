...
public class SeedServiceImpl implements SeedService {
	...
	public Seed updateSeed(Seed seed) throws SeedException {
		...
		check.setDifficultyLevel(
				uv.checker(seed.getDifficultyLevel()) ? seed.getDifficultyLevel() : check.getDifficultyLevel());
		check.setSeedCost(uv.checker(seed.getSeedCost()) ? seed.getSeedCost() : check.getSeedCost());
		check.setSeedDescription(
				uv.checker(seed.getSeedDescription()) ? seed.getSeedDescription() : check.getSeedDescription());
		check.setWatering(uv.checker(seed.getWatering()) ? seed.getWatering() : check.getWatering());
		check.setSeedsPerPacket(
				uv.checker(seed.getSeedsPerPacket()) ? seed.getSeedsPerPacket() : check.getSeedsPerPacket());
